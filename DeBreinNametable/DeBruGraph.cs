...
namespace DeBruijnNametable
{
    public class DeBruGraph
    {
        ...
        {
            Options.nparts = Options.partvars.Length; // Отладка
            parts = new INodePart[Options.nparts];
            //parts[0] = new NodesPart(@"C:\Home\data\deBrein\nlist.bin");
            //parts[0] = new NodesPartNet(sconnection.clients[0]);
            if (Options.partvars[0] == "m")
            {
                parts[0] = new NodesPart(Options.nodelistfilename);
                for (int i = 1; i < Options.partvars.Length; i++) parts[i] = new NodesPartNet(sconnection.clients[i - 1]);
            }
            else
            {
                parts[0] = new NodesPartNet(sconnection.clients[0]);
